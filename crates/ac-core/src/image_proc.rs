...
pub struct DiffRegion {
    ...
    #[pyo3(get)] pub w: u32,
    #[pyo3(get)] pub h: u32,
    #[pyo3(get)] pub changed_pixels: u64,
    #[pyo3(get)] pub total_pixels: u64,
    #[pyo3(get)] pub percent: f64,
}

#[pymethods]
impl DiffRegion {
    #[new] fn new() -> Self { DiffRegion { x: 0, y: 0, w: 0, h: 0, changed_pixels: 0, total_pixels: 0, percent: 0.0 } }
}

#[pyclass]
#[derive(Clone)]
pub struct DiffResult {
    #[pyo3(get)] pub changed: bool,
    #[pyo3(get)] pub percent: f64,
    #[pyo3(get)] pub changed_pixels: u64,
    #[pyo3(get)] pub total_pixels: u64,
}
...
#[pymethods]
impl DiffResult {
    #[new] fn new() -> Self { DiffResult { changed: false, percent: 0.0, changed_pixels: 0, total_pixels: 0 } }
}
...
pub fn image_diff(
    ...
    if before_rgba.len() != expected || after_rgba.len() != expected {
        return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>("RGBA buffer size mismatch"));
    }
    ...
    Ok(DiffResult {
        changed: changed > 0,
        percent: if total > 0 { changed as f64 / total as f64 * 100.0 } else { 0.0 },
...
pub fn dhash(rgba: Vec<u8>, width: u32, height: u32) -> PyResult<u64> {
    if rgba.len() < (width * height * 4) as usize {
        return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>("Buffer too small"));
    }
...
#[pymethods]
impl MatchResult {
    #[new] fn new() -> Self { MatchResult { x: 0, y: 0, confidence: 0.0 } }
}
...
pub fn template_match(
    ...
    for y in (0..=img_h - tmpl_h).step_by(step as usize) {
        for x in (0..=img_w - tmpl_w).step_by(step as usize) {
            ...
            if score > 0.999 {
                return Ok(Some(MatchResult { x: best_x, y: best_y, confidence: best_score }));
            }
        }
    }

    if best_score >= threshold {
        Ok(Some(MatchResult { x: best_x, y: best_y, confidence: best_score }))
    } else {
        Ok(None)
    }
}
...
pub fn has_changed(
    ...
    let diff = image_diff(before_rgba.clone(), after_rgba.clone(), width, height, pixel_threshold)?;
...
mod tests {
    ...
    fn black_rgba(w: u32, h: u32) -> Vec<u8> { vec![0u8; (w * h * 4) as usize] }
    fn white_rgba(w: u32, h: u32) -> Vec<u8> { vec![255u8; (w * h * 4) as usize] }
    fn small_img() -> Vec<u8> {
        let mut v = vec![0u8; 32*32*4];
        for i in (0..v.len()).step_by(4) { v[i]=128; v[i+1]=100; v[i+2]=80; v[i+3]=255; }
        v
    }
    ...
    fn test_dhash_different() {
        ...
        for y in 0..32u32 { for x in 0..32u32 {
            ...
            let v = (x * 8) as u8;
            grad[i]=v; grad[i+3]=255;
            flat[i]=128; flat[i+3]=255;
        }}
    ...
    #[test]
    fn test_hamming_zero() { assert_eq!(hamming_distance(0, 0), 0); }
    #[test]
    fn test_hamming_identical() { assert_eq!(hamming_distance(0xFFFFFFFFFFFFFFFF, 0xFFFFFFFFFFFFFFFF), 0); }
    #[test]
    fn test_hamming_diff() { assert!(hamming_distance(0xFFFFFFFFFFFFFFFF, 0) > 0); }
    ...
    fn test_template_match_none_small() {
        let r = template_match(black_rgba(10,10), 10, 10, black_rgba(20,20), 20, 20, None).expect("match");
